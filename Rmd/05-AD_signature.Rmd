...
heatmap_annotation <- 
  HeatmapAnnotation(
    `Tissue source` = colData(se_qc_deg) %>% as_tibble() %>% pull(skin_type), 
    `Anatomic region` = colData(se_qc_deg) %>% as_tibble() %>% pull(biopsy_area),
    col = list(`Tissue source` = c("HC" = "#91cf60",
                                   "NL" = "#eb8b9b", 
                                   "LS" = "#eb2d0c"),
               `Anatomic region` = sample_site_color),
    annotation_legend_param = 
      list(`Tissue source` = list(nrow = 1, title_gp = gpar(fontsize = 12),
...
jpeg(filename = "../figure/heatmap_DEG.jpeg", width = 720*4, 

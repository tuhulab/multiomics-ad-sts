...
pca_plot <-
  ...
  rename(`Sequencing depth` = qc_coverage,
         `Tissue source` = skin_type) %>% 
  ggscatter(x = "PC1", y = "PC2", 
            color = "Sequencing depth", 
            shape = "Tissue source", font.legend = c(11, "plain", "black"))

ggsave("../data/pca.jpeg", pca_plot, width = 6.2, height = 5)
```
...
heatmap_annotation <- 
  HeatmapAnnotation(
    `Tissue source` = colData(se_heatmap) %>% as_tibble() %>% pull(skin_type), 
    `Anatomic region` = colData(se_heatmap) %>% as_tibble() %>% pull(biopsy_area),
    col = list(`Tissue source` = pca_color,
               `Anatomic region` = sample_site_color),
    annotation_legend_param = 
      list(`Tissue source` = list(nrow = 1, title_gp = gpar(fontsize = 16),
